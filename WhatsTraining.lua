...
local learnedSpellEvent = wt.isLegacyBC and "SPELLS_CHANGED" or ((WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC) and "LEARNED_SPELL_IN_SKILL_LINE" or "LEARNED_SPELL_IN_TAB")
...
local function isPreviouslyLearnedAbility(spellId)
    ...
    for i, otherId in ipairs(wt.overriddenSpellsMap[spellId]) do
        ...
        if wt.IsSpellKnown(otherId) then
            knownIndex = i
        end
    end
