...
if wt.isLegacyBC then
    wt.WarlockPetSummonIds[713] = nil

    local legacyPetOrder = {}
    for _, family in ipairs(wt.WarlockPetOrder) do
        if family ~= "Incubus" then
            tinsert(legacyPetOrder, family)
        end
    end
    wt.WarlockPetOrder = legacyPetOrder

    for level, tomesAtLevel in pairs(wt.TomesByLevel) do
        local legacyTomes = {}
        for _, tome in ipairs(tomesAtLevel) do
            if tome.family ~= "Incubus" then
                if string.find(tome.id or "", "^SUC:") then
                    tome.id = nil
                end
                tinsert(legacyTomes, tome)
            end
        end
        wt.TomesByLevel[level] = legacyTomes
    end
end
