...
local function checkCurrentPetSpells()
    ...
    for _, tome in ipairs(familyTomes) do
        if wt.IsSpellKnown(tome.taughtSpell, true) then
...
events:SetScript("OnEvent", function(self, event, arg1, ...)
    if event == "PET_DISMISS_START" then
        wt.InvalidateKnownSpellCache()
        checkCurrentPetSpells()
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" and arg1 == "player" then
        local spellId = select(2, ...)
        if wt.WarlockPetSummonIds[spellId] then
            wt.InvalidateKnownSpellCache()
            wt.After(0.5, checkCurrentPetSpells)
        end
    elseif event == "PLAYER_ENTERING_WORLD" then
        wt.InvalidateKnownSpellCache()
        wt.RunNextFrame(checkCurrentPetSpells)
    end
end)
...
local function coloredStatusTip(key)
    ...
    if wt:IsPetAbilityLearned(key) then
        return wt.ColorText(RED_FONT_COLOR_CODE, spaced(tome.localFamily, ITEM_SPELL_KNOWN))
    elseif ignoreStore:IsIgnored(key) then
        return wt.ColorText(LIGHTYELLOW_FONT_COLOR_CODE, spaced(tome.localFamily, wt.L.IGNORED_HEADER))
    end
    return wt.ColorText(HIGHLIGHT_FONT_COLOR_CODE, spaced(tome.localFamily, wt.L.NOT_KNOWN))
end
...
local function updateMerchantFrame()
    ...
    for i = 1, MERCHANT_ITEMS_PER_PAGE do
        ...
        if index <= numMerchantItems then
            local merchantItemID = wt.GetMerchantItemID(index)
...
local function showSpellTooltip(tt, itemId)
    ...
    wt.SetTooltipSpellByID(tomeSpellTooltip, taughtSpell)
end
hooksecurefunc(GameTooltip, "SetMerchantItem", function(tt, index)
    ...
    if wt.SayaadTomes[merchantItemID] then
        ...
        if wt:IsPetAbilityLearned(merchantItemID) then 
            addStatusTooltip(tt, wt.ColorText(RED_FONT_COLOR_CODE, spaced(tomes[merchantItemID].localFamily, status(merchantItemID))))
        elseif ignoreStore:IsIgnored(merchantItemID) then
            addStatusTooltip(tt, wt.ColorText(LIGHTYELLOW_FONT_COLOR_CODE, spaced(tomes[merchantItemID].localFamily, status(merchantItemID))))
        end
    end
